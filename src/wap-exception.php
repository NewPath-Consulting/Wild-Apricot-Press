...
{
    public const EXCEPTION_OPTION = 'wawp-exception-type';
    /**
...
class API_Exception extends Exception
{
     ...
     * @var string
     */
    public const ERROR_DESCRIPTION = 'connecting to WildApricot';
...
class Encryption_Exception extends Exception
{
     ...
     * @var string
     */
    public const ERROR_DESCRIPTION = 'encrypting your data';
...
{
    public const ERROR_DESCRIPTION = 'decrypting your data';

    public function __construct($message = '', $code = 0, ?\Throwable $previous = null)
    {
        if (empty($message)) {
            $message = self::decrypt_error();
        }
        parent::__construct($message, $code, $previous);
        if (!Addon::is_plugin_disabled()) {
            update_option(Addon::WAWP_DISABLED_OPTION, true);
